...
import { OrderData, mergeOrders } from "@/lib/order-types";
      ...
      if (supabase && orders.length > 0) {
        try {
          const rows = orders.map((o: OrderData) => ({
            id: o.id,
            code: o.code || o.id,
            official_code: o.officialCode || o.code || o.id,
            temp_code: o.tempCode || o.code || o.id,
            customer_name: o.customerName || "Pelanggan",
            whatsapp: o.whatsapp || "000000000000",
            instagram: o.instagram || "@pelanggan",
            package: o.package || "Pertamax",
            photo_count: o.photoCount || 1,
      ...
      if (supabase) {
        try {
          const { error: upsertErr } = await supabase.from("orders").upsert({
            ...
            official_code: order.officialCode || order.code || order.id,
            temp_code: order.tempCode || order.code || order.id,
            customer_name: order.customerName || "Pelanggan",
            whatsapp: order.whatsapp || "000000000000",
            instagram: order.instagram || "@pelanggan",
            package: order.package || "Pertamax",
            photo_count: order.photoCount || 1,
